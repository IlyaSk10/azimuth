...
for depth_ind in range(submodels_start, submodels_end + 1, submodels_step):  # for submodels
    ...
    Z1_ind, Z2_ind = depth_ind - delta_z, depth_ind + delta_z

    if (depth_ind - delta_z) < 0:
        Z1_ind = 0
        Z2_ind = (depth_ind + delta_z) + np.abs(depth_ind - delta_z)

    if (depth_ind + delta_z) > half_well.shape[0] - 1:
        Z2_ind = half_well.shape[0] - 1
        Z1_ind = (depth_ind - delta_z) - np.abs((depth_ind + delta_z) - (half_well.shape[0] - 1))
    ...
    for wave in pkgs_names:
        ...
        for s in range(Gtsum.shape[0]):
            ...
            for chan in num_channels:
                for source_p in range(Gtsum.shape[1]):
                    ...
                    dist = np.sqrt((sensors_coords[s, 0] - sources_coords[source_p, 0]) ** 2 + (
                                sensors_coords[s, 1] - sources_coords[source_p, 1]) ** 2 + (
                                               sensors_coords[s, 2] - sources_coords[source_p, 2]) ** 2)
                    tcenter = dist / v
        ...
        if wave == 'P' and central_model == depth_ind:
            # sens_c=sensors[sub_sensors[sub_sensors.shape[0]//2]]
            # P_wave_full_wave=bachata_obj_sen.get_full_wave_time(sensors=sens_c, channels='Z', subModelName=subModelName, points='all',components='all')
            subModelName_P = subModelName
            bachata_obj_sen_P = BachataClass("bachata_obj_P_temp.hdf5",
        ...
        if wave == 'S' and central_model == depth_ind:
            # sens_c=sensors[sub_sensors[sub_sensors.shape[0]//2]]
            # S_wave_full_wave=bachata_obj_sen.get_full_wave_time(sensors=sens_c, channels='Z', subModelName=subModelName, points='all',components='all')
            subModelName_S = subModelName
            bachata_obj_sen_S = BachataClass("bachata_obj_S_temp.hdf5",
                                             data=data_list, shifts=shifts_list, subModelNames=subModelName_S,
                                             data_type='window_shift_time',
                                             L_win=L_win, L=L, fd=fd, fmin=fmin, fmax=fmax, components=compnts,
                                             sensors=sensors, sub_sensors=sub_sensors, central_Z=depth,
                                             channels=channels, field=field, xcenter=xcenter, ycenter=ycenter,
        ...
        window_shift_frequency_data = bachata_obj_sen.get_window_shift_frequency(
            sensors=list(np.asarray(sensors)[sub_sensors]), channels='all', points='all', components='all',
    ...
    if central_model == depth_ind:
        ...
        for i in range(number_sources):
            ...
            # fig.tight_layout()
            fig.suptitle('central sensor name ' + str(half_well[depth_ind, 0]) + ',  central depth ' + str(
                depth) + ' m, source X dist ' + str(sources_coords[i, 0]) + ' m', horizontalalignment='center')
            ...
            ax.set_xticks([i for i in range(0, fft_dat.shape[1] + 1, 20)],
                          [round((i * fd) / dat.shape[1], 0) for i in range(0, fft_dat.shape[1] + 1, 20)], rotation=90)
            fig.colorbar(im)
            ax.set_aspect(2.5)
            ...
            ax.set_xlabel('frequency, Hz')
            ax.set_xticks([i for i in range(0, fft_dat1.shape[1] + 1, 20)],
                          [round((i * fd) / dat1.shape[1], 0) for i in range(0, fft_dat1.shape[1] + 1, 20)],
                          rotation=90)
            fig.colorbar(im)
            ax.set_aspect(2.5)
            ...
            result.update({'fig, central_sensor_name ' + str(half_well[depth_ind, 0]) + ', depth ' + str(
                depth) + ' m, X_source_dist ' + str(sources_coords[i, 0]) + ' m.png': fig})

        for i in range(number_sources):
            ################################################
            sens_c = delta_z // 2
            sensor_name = sensors[sub_sensors[sens_c]]

            # print(sens_c,sensor_name)

            fig1 = plt.figure(num=str(i) + 'f', figsize=(10, 5), constrained_layout=True, facecolor='white')
            fig1.suptitle('central sensor name ' + str(half_well[depth_ind, 0]) + ' , subsensor name ' + str(
                sensor_name) + ',  central depth ' + str(depth) + ' m, source X dist ' + str(
                sources_coords[i, 0]) + ' m', horizontalalignment='center')

            ax = fig1.add_subplot(1, 2, 1)
            ax.plot(np.sum(Gtp_total_mean[:, :, chan, :3, :], axis=2)[sens_c, i, :] * 10 ** power, label='modelation')
            P_wave_full_wave = bachata_obj_sen_P.get_full_wave_time(sensors=sensor_name, channels='Z',
                                                                    subModelName=subModelName_P, points='all',
                                                                    components='all')
            ax.plot(np.sum(P_wave_full_wave[:, :3, :], axis=1)[i, :], label='after batch filtration')
            ax.set_title('P wave, sum(xx,yy,zz), derivative', fontsize=10)
            # ax.set_aspect(50)
            ax.legend()

            ax = fig1.add_subplot(1, 2, 2)
            ax.plot(Gts_total_mean[sens_c, i, chan, 2, :] * 10 ** power, label='modelation')
            S_wave_full_wave = bachata_obj_sen_S.get_full_wave_time(sensors=sensor_name, channels='Z',
                                                                    subModelName=subModelName_S, points='all',
                                                                    components='all')
            ax.plot(S_wave_full_wave[i, 2, :], label='after batch filtration')
            ax.set_title('S wave, zz, derivative', fontsize=10)
            # ax.set_aspect(50)
            ax.legend()

            result.update({'fig, response, central_sensor_name ' + str(
                half_well[depth_ind, 0]) + ' , subsensor name ' + str(sensor_name) + ', central depth ' + str(
                depth) + ' m, X_source_dist ' + str(sources_coords[i, 0]) + ' m.png': fig1})
print('Done')
...
path = job.save_package(label='loc_system_' + bachata_name, fields=dict(num_subModels=num_subModels),
                        files={'Bachata.hdf5': lambda f: shutil.copy2(new_filename, f.parent)})
