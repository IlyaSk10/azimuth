...
for i in range(inkl_data.shape[0]):
    R = calc_r(inkl_data[i, 5], inkl_data[i, 6])
    MD = inkl_data[i, 0]

    old_well.append([inkl_data[i, 5], MD])

old_well = np.array(old_well)

plt.plot(old_well[:, 0], old_well[:, 1])
plt.xlabel('X')
plt.ylabel('MD')
plt.title('Вид сбоку')
...
# # plt.plot(sensors_depths[:sensors_depths.shape[0] // 2, 1], label ='sensors')
# plt.plot(inkl_data[:, 1], label='TVD')
# plt.plot(inkl_data[:, 0], label='MD')
# plt.gca().invert_yaxis()
# plt.show()
...
for i in range(inkl_data.shape[0] - 1):
    R = calc_r(inkl_data[i + 1, 5] - inkl_data[i, 5], inkl_data[i + 1, 6] - inkl_data[i, 6])
    MD = inkl_data[i + 1, 0] - inkl_data[i, 0]

    R_new = R * math.cos(rad) + MD * math.sin(rad)
    MD_new = R * math.sin(rad) - MD * math.cos(rad)

    new_well.append([R_new, MD_new])

new_well = np.array(new_well)

plt.plot(new_well[:, 0], new_well[:, 1])
plt.xlabel('R')
plt.ylabel('MD_new')
plt.title('Вид сбоку')
plt.gca().invert_yaxis()
